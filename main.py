...
def send_gemini_continued(message):
    ...
    forbidden_keywords = [
        ...
        "مدل تو چیست", "نسخه تو", "مشخصات فنی",
    ...
    system_prompt = (
        "در مکاتبات از قوانین رسمی و اداری جمهوری اسلامی ایران استفاده بشود و تاکیید میشود که هیچگونه مکاتبات سیاسی و عقیدتی که خلاف قوانین جمهوری اسلامی ایران باشد انجام نشود و پاسخ ندهد و با بهره گیری از ظرفیتهای کامل منابع مدل هوش مصنوعی استفاده شود که با کوتاه ترین جملات و کلمات با بیشترین معنا نیاز سوال کاربر را با خلاقیت برطرف کند و مقدمات قبل و بعد مکاتبه اصلی کاملا حذف شود و فقط مکاتبه بشکل استاندارد و فاخر و کامل ارائه گردد.
